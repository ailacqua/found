"""
All locations and their building code provided by Cornell University.
Locations serialized by ChatGPT. There may be minor spelling errors.

Author: Michael Gaile, ChatGPT
Date: 12/1/2023
"""
LOCATIONS = {
    "ACH": "Alice Cook House",
    "ADW": "AD White House",
    "AFC": "Africana Center",
    "AGQ": "AG Quad",
    "AKW": "Akwe:Kon",
    "APP": "Appel Commons",
    "ASA": "Academic Surge A",
    "ASB": "Academic Surge B",
    "ASQ": "Arts Quad",
    "ATH": "Anabel Taylor Hall",
    "BAL": "Balch Hall",
    "BAU": "Bauer Hall",
    "BBE": "Beebe Beach",
    "BBH": "Beebe Hall",
    "BDF": "Bradfield Hall",
    "BET": "Bethe House",
    "BGC": "Botanic Gardens Arboretum",
    "BKL": "Baker Laboratory",
    "BKR": "Becker House",
    "BLM": "Bloomberg Center",
    "BLY": "Bailey Hall",
    "BND": "Bondareff Raptor Facility",
    "BOL": "Boldt Hall",
    "BRB": "Big Red Barn",
    "BRD": "Bard Hall",
    "BRN": "Barnes Hall",
    "BRZ": "Breazzano Family Ctr for BusEd",
    "BSC": "Basic Science Building",
    "BTA": "Baker Institute",
    "BTB": "Biotechnology Building",
    "BTI": "Boyce Thompson Institute",
    "BTL": "Bartels Hall",
    "BTN": "Barton Hall",
    "CAS": "Cascadilla Hall",
    "CAV": "College Ave, 301",
    "CCC": "Computing & Communications Ctr",
    "CGC": "Cornell Golf Center",
    "CKB": "Court Kay Bauer Lawn",
    "CLA": "Clinical Programs - Ambulatory",
    "CLB": "Clinical Programs - Breezeway",
    "CLD": "Caldwell Hall",
    "CLK": "Clark Hall",
    "CLM": "Clinical Programs M Barn",
    "CLS": "Clinical Programs - Surgery",
    "CLU": "Clinical Programs - Multipurpose",
    "CMH": "Corson & Mudd Halls",
    "CMS": "Comstock Hall",
    "CNH": "Cornell Health",
    "COM": "Anna Comstock House",
    "CPK": "Clinical Programs K Barn",
    "CRP": "Carpenter Hall",
    "CRT": "Court Residence Hall",
    "CVM": "CVM Center",
    "DAY": "Day Hall",
    "DCK": "Clara Dickson Hall",
    "DON": "Mary Donlon Hall",
    "DUF": "Duffield Hall",
    "ECO": "Hurlburt House",
    "EMR": "Emerson Hall",
    "ENQ": "Engineering Quad",
    "ESN": "Judith Eisner Pavilion",
    "FBC": "Fischell Band Center",
    "FND": "Foundry",
    "FNL": "Newman Laboratory",
    "FRH": "Flora Rose House",
    "FRN": "Fernow Hall",
    "FUE": "Fuertes Observatory",
    "FWC": "Friedman Wrestling Center",
    "GBL": "Gnva Barton Laboratory",
    "GGG": "Green Greenhouse Bldg G",
    "GGM": "Green Greenhouse Bldg M",
    "GRS": "Grumman Squash Courts",
    "GSH": "Goldwin Smith Hall",
    "GTR": "Guterman Bioclimatic Lab",
    "GTS": "Bill and Melinda Gates Hall",
    "HCC": "Hasbrouck Community Center",
    "HEB": "Human Ecology Building",
    "HGH": "Hughes Hall",
    "HLS": "Hollister Hall",
    "HNH": "Helen Newman Hall",
    "HOP": "Ho Plaza",
    "HR5": "High Rise #5",
    "HVL": "High Voltage Laboratory",
    "ICC": "ILR Conference Center",
    "IRB": "ILR Research Building",
    "IVF": "Ives Hall Faculty Wing",
    "IVS": "Ives Hall",
    "JLO": "Ornithology, Cornell Lab of",
    "JMA": "Johnson Museum Of Art",
    "JMS": "George Jameson Hall",
    "JSS": "Jessup Field",
    "KAY": "Kay Hall",
    "KHN": "Stewart Ave 640, Kahin Center",
    "KLR": "Klarman Hall",
    "KMB": "Kimball Hall",
    "KND": "Kennedy Hall",
    "KRC": "Kroch, Carl A Library",
    "KSH": "West Ave 104",
    "KTN": "William T. Keeton House",
    "LBS": "Library Slope",
    "LDL": "Liddell Laboratory",
    "LLB": "Plant Breeding Love Laboratory",
    "LNC": "Lincoln Hall",
    "LNH": "Lynah Rink",
    "LR6": "Low Rise #6",
    "LR7": "Low Rise #7",
    "LR8": "Low Rise #8",
    "LR9": "Low Rise #9",
    "LSP": "Livestock Pavilion",
    "LWS": "Botanic Gardens RM Lewis Educ Center",
    "LXT": "Lexington Ave 570",
    "MCC": "McConville Barn",
    "MCF": "Mcfaddin Hall",
    "MCG": "McGraw Hall",
    "MCL": "McClintock",
    "MEW": "Mews Residence Hall",
    "MKL": "Moakley House - Golf Course",
    "MLS": "Milstein Hall",
    "MLT": "Malott Hall",
    "MNL": "Mann Library",
    "MNS": "Muenscher Laboratory",
    "MRB": "Moore Laboratory",
    "MRL": "Morrill Hall",
    "MRS": "Morrison Hall",
    "MSC": "Merrill Family Sailing Center",
    "MTH": "Myron Taylor Hall",
    "MVR": "M Van Rensselaer Hall",
    "MWS": "Mews Hall Courtyard",
    "NLG": "Noyes Lodge",
    "NCT": "North Campus Turf Field",
    "NRC": "Noyes Community and Rec Center",
    "NVN": "Nevin Welcome Center",
    "OLC": "Olin Chemistry Research Wing",
    "OLH": "Olin Hall",
    "OLL": "Olin Library",
    "OXL": "John T Oxley Equestrian Center",
    "PCS": "Pomology Cold Storage Sales",
    "PHL": "Phillips Hall",
    "PHS": "Physical Sciences Building",
    "PLS": "Plant Science Building",
    "PPG": "Poisonous Plant Garden",
    "RBT": "Roberts Hall",
    "RCE": "Rice Hall",
    "RCK": "Rockefeller Hall",
    "RCP": "Recreation Club Pavilion",
    "RHD": "Frank H T Rhodes Hall",
    "RIS": "Risley Lawn",
    "RND": "Rand Hall",
    "RPC": "Robert Purcell Community Center",
    "RRB": "Riley-Robb Hall",
    "RSL": "Risley, Prudence Resd. College",
    "RST": "Risley Tennis Court",
    "RTC": "Reis Tennis Center",
    "RWL": "Rawlings Green",
    "SBL": "Sibley Hall",
    "SCT": "Sheldon Court",
    "SGC": "Sage Chapel",
    "SGH": "Sage Hall",
    "SHK": "Schoellkopf Memorial Hall",
    "SKF": "Schoellkopf Field",
    "SKT": "Schoellkopf Track",
    "SNE": "Snee Hall",
    "SRM": "Schurman Hall",
    "SSB": "Space Science Building",
    "STK": "Stocking Hall",
    "STL": "Statler Hall",
    "STM": "Stimson Hall",
    "SVG": "Savage Hall",
    "SWZ": "Schwartz Center",
    "TCC": "Townhouse Community Center",
    "TDB": "Teaching Dairy Barn",
    "TGL": "Teagle Hall",
    "TIC": "Tata Innovation Center",
    "THA": "Thurston Avenue 626",
    "THR": "Thurston Hall",
    "TJN": "Olive Tjaden Hall",
    "TMR": "Toni Morrison Hall",
    "TRB": "Teaching Research Barn",
    "UJM": "Ujamaa",
    "UNI": "University Ave 726",
    "UPS": "Upson Hall",
    "URH": "Uris Hall",
    "URL": "Uris Library",
    "VCM": "Vet Center For Mobility",
    "VDL": "NYS Veterinary Diagnostic Lab",
    "VEC": "Vet Education Center",
    "VMC": "Vet Medical Center",
    "VRT": "Vet Research Tower",
    "VRZ": "Verizon Exec Education Center",
    "WHT": "White Hall",
    "WLL": "Weill Hall",
    "WLP": "Wolpe Center",
    "WNG": "Wing Hall",
    "WRD": "Ward Center",
    "WRM": "War Memorial"
}
